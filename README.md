...
- Node.js is available on your machine
- your Codex installation supports `codex mcp add`
- the Forge AI plugin is active
...
9. verify the registration with `codex mcp list`
10. open Codex on the same workspace and run `get_snapshot`
...
If your Codex build does not expose `codex mcp add`, register the same stdio command manually in the Codex MCP settings using the same environment variables.

...
class WindPressCompiler {
  ...
  async buildCacheInternal(options = {}) {
    ...
    return {
      ...
      plugins,
      changeset: stored ? (stored.result || stored).changeset || null : null,
      rollback_available: stored ? (stored.result || stored).rollback_available === true : false,
      undo_tool: stored ? (stored.result || stored).undo_tool || '' : '',
      compilation_evidence: { compiler: 'installed_windpress_runtime', compiled_locally: true, source_revision: status.source_revision, plugins },

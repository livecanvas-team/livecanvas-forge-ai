...
package_assert_true(in_array('livecanvas-forge-ai/docs/release-notes-0.2.0-beta.8.md', $entries, true), 'distribution zip should include release notes with current qualification limits');
...
package_assert_true(strpos($plugin_bootstrap, 'Version: 0.2.0-beta.8') !== false, 'distribution zip should identify the beta release build');
...
package_assert_true(strpos($plugin_bootstrap, "define('LCFA_MCP_PACKAGE_VERSION', '0.2.0-beta.8')") !== false, 'distribution zip should pin the matching MCP package');
package_assert_true(in_array('livecanvas-forge-ai/assets/runtime/livecanvas-ai-bridge-mcp-0.2.0-beta.8.tgz', $entries, true), 'distribution must contain the actual installer archive used by generated prompts');
package_assert_true(in_array('livecanvas-forge-ai/includes/class-lcfa-site-knowledge.php', $entries, true), 'distribution must include explicit administrator-reviewed site instructions');
package_assert_true(in_array('livecanvas-forge-ai/docs/site-knowledge.md', $entries, true), 'distribution must explain sharing and withdrawing site instructions');
package_assert_true(in_array('livecanvas-forge-ai/includes/class-lcfa-desktop-delivery.php', $entries, true), 'distribution must include the private delivery journal');
package_assert_true(in_array('livecanvas-forge-ai/assets/editor-buffer.js', $entries, true), 'distribution must ship the fail-closed native editor guard');
package_assert_true(in_array('livecanvas-forge-ai/includes/class-lcfa-session-store.php', $entries, true), 'distribution must include concurrent revocation-safe storage');
package_assert_true(in_array('livecanvas-forge-ai/docs/local-transport.md', $entries, true), 'distribution must explain authenticated transport and desktop limits');
package_assert_true(in_array('livecanvas-forge-ai/includes/class-lcfa-file-changesets.php', $entries, true), 'distribution must include the shared file journal');
package_assert_true(in_array('livecanvas-forge-ai/includes/class-lcfa-picostrap-changesets.php', $entries, true), 'distribution must include the guarded bundle journal');
package_assert_true(in_array('livecanvas-forge-ai/includes/class-lcfa-windpress-changesets.php', $entries, true), 'distribution must include the compiled cache journal');
package_assert_true(in_array('livecanvas-forge-ai/docs/windpress-assets.md', $entries, true), 'distribution must include compiled cache Undo instructions');
package_assert_true(in_array('livecanvas-forge-ai/docs/picostrap-assets.md', $entries, true), 'distribution must include bundle Undo instructions');
package_assert_true(in_array('livecanvas-forge-ai/docs/file-changesets.md', $entries, true), 'distribution must include file Undo instructions and limits');
...
package_assert_true(in_array('livecanvas-forge-ai/includes/class-lcfa-workflows.php', $entries, true), 'distribution must include the shared workflow service');
package_assert_true(in_array('livecanvas-forge-ai/includes/class-lcfa-changesets.php', $entries, true), 'distribution must include private changeset storage and Undo');
package_assert_true(in_array('livecanvas-forge-ai/includes/class-lcfa-private-store.php', $entries, true), 'distribution must include private owner-scoped storage');
package_assert_true(in_array('livecanvas-forge-ai/includes/class-lcfa-private-chat.php', $entries, true), 'distribution must include atomic private queue behavior');
package_assert_true(strpos($plugin_bootstrap, "require_once LCFA_DIR . 'includes/class-lcfa-private-chat.php'") !== false, 'the production bootstrap must load private storage instead of the isolated legacy fixture path');
package_assert_true(in_array('livecanvas-forge-ai/workflows/catalog.json', $entries, true), 'distribution must include workflow discovery');
foreach (['shared', 'page', 'shared-component', 'dynamic-template', 'assets', 'verify-restore'] as $workflow) {
    package_assert_true(in_array('livecanvas-forge-ai/workflows/' . $workflow . '.md', $entries, true), 'distribution must include workflow body: ' . $workflow);
}
package_assert_true(in_array('livecanvas-forge-ai/skills/livecanvas-site/SKILL.md', $entries, true), 'distribution must include the portable agent skill');
...
package_assert_true(in_array('livecanvas-forge-ai/docs/content-changesets.md', $entries, true), 'distribution must include changeset coverage and Undo instructions');
package_assert_true(in_array('livecanvas-forge-ai/docs/private-conversations.md', $entries, true), 'distribution must include private conversation migration and protocol instructions');
package_assert_true(in_array('livecanvas-forge-ai/docs/release-notes-0.2.0-beta.8.md', $entries, true), 'distribution must include the beta.8 release notes');
